...
func SOCKS5(network, addr string, auth *Auth, forward Dialer, timeout time.Duration) (Dialer, error) {
	s := &socks5{
		network: network,
		addr:    addr,
		forward: forward,
		timeout: timeout, // add by biter
	}
	if auth != nil {
		s.user = auth.User
...
type socks5 struct {
	...
	network, addr  string
	forward        Dialer
	timeout        time.Duration // add by biter
}
